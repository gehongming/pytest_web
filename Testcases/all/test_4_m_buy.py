...
#密码登录
    ...
    def test_login(self,open_url_m):
        try:
            lp(open_url_m).login_pwd(bd.login_data["user"],bd.login_data["passwd"])
            time.sleep(1)
            assert True == ip(open_url_m).login_check()
            assert unquote(open_url_m.current_url, encoding='utf-8') == 'https://m.yjq.com/'
...
#隐形眼镜加入购物车
    ...
    def test_buy_yx(self,open_url_m):
        ...
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8')== bd.success_data_yxyj["check_url"]
        #点击进入商品详情页
        hp(open_url_m).click_yx()
        #验证是否进入
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8')==bd.success_data_yxyj["check_product_url"]
        #选择sku
        pp(open_url_m).yxyj()
        time.sleep(0.5)
...
#护理液加入购物车
    ...
    def test_buy_hl(self, open_url_m):
        ...
        time.sleep(2)
        # 是否进入搜索结果页
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_huliye["check_url"]
        # 点击进入商品详情页
        hp(open_url_m).click_hl()
        # 验证是否进入
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_huliye["check_product_url"]
        # 选择数量并加入购物车
...
#定制片加入购物车
    ...
    def test_buy_dzp(self, open_url_m):
        ...
        time.sleep(1)
        # 是否进入搜索结果页
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_dzp["check_url"]
        # 点击进入商品详情页
        hp(open_url_m).click_dz()
        # 验证是否进入
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_dzp["check_product_url"]
        # 选择sku并加入购物车
        pp(open_url_m).dingzhi(bd.success_data_dzp["sku"][0],bd.success_data_dzp["sku"][1],bd.success_data_dzp["sku"][2],
...
#多彩伴侣盒加入购物车
    ...
    def test_buy_blh(self, open_url_m):
        ...
        time.sleep(1)
        # 是否进入搜索结果页
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_blh["check_url"]
        # 点击进入商品详情页,并切换窗口
        hp(open_url_m).click_blh()
        # 验证是否进入
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_blh["check_product_url"]
        # 选择sku并加入购物车
...
#镜片加入购物车
    ...
    def test_buy_jp(self, open_url_m):
        ...
        time.sleep(2)
        # 是否进入搜索结果页
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_jp["check_url"]
        # 点击进入商品详情页,并切换窗口
        hp(open_url_m).click_jp()
        # 验证是否进入
        time.sleep(1)
        assert unquote(open_url_m.current_url, encoding='utf-8') == bd.success_data_jp["check_product_url"]
        # 选择sku并加入购物车
...
class TestXiajia:
    ...
    def test_login(self, open_url_pc):
        try:
            lpc(open_url_pc).login_code(bdc.login_data["user"], bdc.login_data["passwd"])
            time.sleep(5)
    ...
    def test_seller_product(self, open_url_pc):
        ...
        time.sleep(1.5)
        assert unquote(open_url_pc.current_url,
